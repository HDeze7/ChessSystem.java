...
public class ChessMatch {
	...
	public ChessPiece[][] getPieces() {// Ele vai ter que me retornar uma matriz de peça de Xadrez, correspondente à
										// essa partida.
		ChessPiece[][] mat = new ChessPiece[board.getRows()][board.getColumns()];// Vou criar aqui, temporariamente uma
																					// variável auxiliar do tipo
																					// ChessPiece matriz, vou iniciar
																					// ela com um new
		...
		for (int i = 0; i < board.getRows(); i++) {// Eu faço um "for" normal para percorrer as linhas("i") da matriz
			for (int j = 0; j < board.getColumns(); j++) {// e dentro desse "for", vai ter um outro "for" para percorrer
															// as colunas da matriz que vai ser correspondente à "j"
				mat[i][j] = (ChessPiece) board.piece(i, j);// Então pra cada posição "i" "j" do meu tabuleiro, eu vou
															// fazer que a minha matriz mat, na linha i, coluna j ,
															// receba um "board.piece(i,j)", porém antes desse argumento
															// eu faço um downcasting((ChessPiece)) para ChessPiece,
															// porque aí sim ele vai interpretar essa peça como uma peça
															// de xadrez, e não como uma peça comum
			}
		}
